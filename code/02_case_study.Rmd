...
# time = getZ(sla)
# sla = t(sla)
# sla = raster::flip(sla,  direction = "y")
# extent(sla) = e
# sla = setZ(sla, z = time, name = "time")
# 
# e    = extent(fsle)
# time = getZ(fsle)
# fsle = fsle * -1
...
# 
# idx = which(is.element(ts, tc))
# tf  = subset(tf, is.element(ts, tc))
# sla = raster::subset(sla, idx)
# sla = setZ(sla, z = as.Date(ts), name = "time")
# 
# rm(tc, tf, idx, t, e)
# gc()
```
...
cropping them to the same extent
```{r crop}
# e = extent(-165, -130, 23, 35)
# 
...
Making all same size and shape
```{r resample}
...
# fsle = setZ(fsle, z = as.Date(tf), name = "time")
# sla  = setZ(sla,  z = as.Date(ts), name = "time")
# chl  = setZ(chl,  z = as.Date(tc), name = "time")
```
...
# fsle_2018 = timesnip(fsle,  as.Date(sdate), as.Date(edate))
# sla_2018  = timesnip(sla, as.Date(sdate), as.Date(edate))

# cleaning work space
#rm(chl, chla, chlb, idx, sdate, edate)
```

Create table
```{r}
# this is much simpler code but may take alot of memory
# xyz = tableit(c(s, f, c, cb, ca))
# colnames(xyz) = c("lon", "lat", "time", "sla", "fsle", "chl", "chlb", "chla")
# 
# xyz$fsle = xyz$fsle * -1
# #xyz = subset(xyz, !fsle == 0)
# 
# xyz$time = as.character(xyz$time)
# xyz$time = as.numeric(xyz$time)
