...
bloomspan = function(anom, time = getZ(chl), gaps = 1, dur = 1){
  # apply loess to 
...
sumstats = function(blooms = chlb, signal = chl, tbl){
  ...
  sumtbl = data.frame(sdate    = tbl$sdate,
                      edate    = tbl$edate,
...
```{r, message = FALSE, warning = FALSE}
durations = bloomspan(ts$anom, time = getZ(chl), gaps = 7, dur = 5)
gc()
```
...
write.csv(tbl, file = "../data/outfiles/full_sum_2.csv")
gc()
```
