...
load_nc = function(path = "C:\\Users\\james\\Desktop\\jamieslife\\data\\infiles\\fsle\\",
  ...
  file  = nc_open(infile)
  lat   = ncvar_get(file, varid = vars[[1]])
  lon   = ncvar_get(file, varid = vars[[2]])
  ras   = ncvar_get(file, varid = vars[[3]])
  t     = ncvar_get(file, varid = vars[[4]])
...
map2   = function(val, 
  ...
  if(is.character(boundry)) {
    # I designate two transect lines by long/lat
    cds1 <- rbind(c(-158, 22), c(-158, 28))
    cds2 <- rbind(c(-147, 22), c(-147, 28))
    cds3 <- rbind(c(-158, 28), c(-147, 28))
    cds4 <- rbind(c(-158, 22), c(-147, 22))
    
    cds5 <- rbind(c(-165, 28.5), c(-165, 34.5))
    cds6 <- rbind(c(-132, 28.5), c(-132, 34.5))
    cds7 <- rbind(c(-165, 28.5), c(-132, 28.5))
    cds8 <- rbind(c(-165, 34.5), c(-132, 34.5))
    
    if(boundry == "aloha") {    
      transects <- SpatialLines(list(Lines(list(Line(cds1)), ID = "one"), 
                                     Lines(list(Line(cds2)), ID = "two"),
                                    Lines(list(Line(cds3)), ID = "three"),
                                    Lines(list(Line(cds4)), ID = "four")))
    }
    if(boundry == "30N") {    
      transects <- SpatialLines(list(Lines(list(Line(cds5)), ID = "five"),
                                    Lines(list(Line(cds6)), ID = "six"),
                                    Lines(list(Line(cds7)), ID = "seven"),
                                    Lines(list(Line(cds8)), ID = "eight")))
      }
    if(boundry == "both") {    
      transects <- SpatialLines(list(Lines(list(Line(cds1)), ID = "one"), 
                                     Lines(list(Line(cds2)), ID = "two"),
  ...
  if(xaxes == TRUE) axis(side = 1, 
                         ...
                         lwd = 2,
                         mgp = c(2, 1, 0),    
                         #at = c(-165, -155, -145, -135),
                         cex.axis = cex.axis)
  box(which = "plot", lty = "solid", lwd = 3, col = "grey25")
  if(is.character(boundry)) {
    plot(transects, lwd = 2, add = TRUE, col = alpha(c("firebrick4"), 0.7))
    }
  plot(wdmap, 
       xlim = e[1:2], 
       ylim = e[3:4], 
